...
- Bundled `i-have-adhd` output-style skill, vendored verbatim from [ayghri/i-have-adhd](https://github.com/ayghri/i-have-adhd) (MIT) under `skills/i-have-adhd/`. `install.sh` now installs it for every selected target — `~/.claude/skills/i-have-adhd/`, `~/.codex/skills/i-have-adhd/` (including its `agents/openai.yaml` metadata), and the Gemini extension's `skills/i-have-adhd/`. Invoke with `/i-have-adhd` (Claude) or `$i-have-adhd` (Codex); "stop adhd mode" turns it off.
